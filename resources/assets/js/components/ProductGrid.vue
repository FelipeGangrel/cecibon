    ...
    <div class="col-lg-12">
      ...
      <section class="panel">
        <div class="panel-body">
          <table class="table table-bordered table-striped">
            <thead>
              <tr>
                <th v-for="key in gridColumns" v-bind:key="key" @click="sortBy(key)" v-bind:class="{active: sortKey == key}">
                  {{ key }}
                  <span class="arrow" v-bind:class="sortOrder > 0 ? 'asc' : 'dsc'"></span>
                </th>
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="row in gridData" v-bind:key="row">
                <td>{{ row.Id }}</td>
                <td><a v-bind:href="'/produto/' + row.Id "> {{ row.Nome }}</a></td>
                <td>{{ row.Categoria }}</td>
                <td>{{ row.Criado }}</td>
                <td>
                  <a v-bind:href="'/produto/' + row.Id + '/edit'">
      ...
      <div class="row">
        <div class="pull-right for-page-button">
          <button @click="getData(go_to_page)" class="btn btn-default">Go To Page:</button>
        ...
        <ul class="pagination pull-right">
          ...
          <li v-for="page in pages" v-bind:key="page" v-if="page > current_page - 2 && page < current_page + 2" v-bind:class="{'active': checkPage(page)}">
        ...
        if (this.query == '' && getPage != null) {
          ...
          if (getPage != null) {
            $.getJSON(getPage, function (data) {
              ...
              this.total = data.total;
              this.last_page = data.last_page;
              this.next_page_url = (data.next_page_url == null) ? null : data.next_page_url +
                '&keyword=' + this.query;
              this.prev_page_url = (data.prev_page_url == null) ? null : data.prev_page_url +
                '&keyword=' + this.query;
              this.first_page_url = '/api/products-data?page=1&keyword=' + this.query;
              this.last_page_url = '/api/products-data?page=' + this.last_page + '&keyword=' +
                this.query;
              this.current_page = data.current_page;
